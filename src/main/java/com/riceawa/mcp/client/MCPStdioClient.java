        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (result != null) {
                    ...
                    logger.info("成功连接到 MCP STDIO 服务器: " + config.getName() + " (版本: " + result.protocolVersion() + ")");
                    ...
                    logger.error("MCP STDIO 服务器初始化失败: " + config.getName());
                    return false;
                }
                
            } catch (Exception e) {
                logger.error("连接 MCP STDIO 服务器失败: " + config.getName() + " - " + e.getMessage());
                connected = false;
                return false;
            }
        });
    ...
    public void disconnect() {
        if (mcpClient != null && connected) {
            try {
                ...
                logger.info("已断开 MCP STDIO 服务器连接: " + config.getName());
            } catch (Exception e) {
                logger.error("断开 MCP STDIO 服务器连接时出错: " + config.getName() + " - " + e.getMessage());
            } finally {
                connected = false;
                mcpClient = null;
            }
